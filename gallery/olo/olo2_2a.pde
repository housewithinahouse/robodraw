...
//  olo.console();        //|
 //------------ ----------| 
  ...
  olo.setValues(  
  map(sliders[0].value(),0,1, -250,350),  //decent
  map(sliders[1].value(),0,1,  1,18), 
  map(sliders[2].value(),0,1,  1,50),
  map(sliders[3].value(),0,1,  1,400),
  map(sliders[4].value(),0,1,  .01,4),
  map(sliders[5].value(),0,1, -50,58), 
  map(sliders[6].value(),0,1, -8,9), 
  ...
  //had a lot of trouble around this part. Processing.js doesn't like overloaded methods. I think that the first method named is how it works. Just having assignment works fine. 

  for (int i = 0; i < 7; i++){

    if(sliders[i].active==true){
     if(kh.right){
      sliders[i].update(1);
     } else if(kh.left){
      sliders[i].update(-1);
